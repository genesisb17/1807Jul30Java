...
BEGIN
    INSERT INTO ERS_REIMBURSEMENT (REIMB_AMOUNT, REIMB_SUBMITTED, REIMB_DESCRIPTION, REIMB_AUTHOR, REIMB_STATUS_ID, REIMB_TYPE_ID)
        VALUES (amount, CURRENT_TIMESTAMP, text, authorId, 1, typeId);
END;
/
CREATE OR REPLACE PROCEDURE approvereimbursement (rId IN NUMBER, resolverId IN NUMBER)
IS
BEGIN
    UPDATE ERS_REIMBURSEMENT SET REIMB_STATUS_ID = 2, REIMB_RESOLVER = resolverId, REIMB_RESOLVED = CURRENT_TIMESTAMP WHERE REIMB_ID = rId;
END;
/
CREATE OR REPLACE PROCEDURE denyreimbursement (rId IN NUMBER, resolverId IN NUMBER)
IS
BEGIN
    UPDATE ERS_REIMBURSEMENT SET REIMB_STATUS_ID = 3, REIMB_RESOLVER = resolverId, REIMB_RESOLVED = CURRENT_TIMESTAMP WHERE REIMB_ID = rId;
END;
/
COMMIT;
EXIT;
