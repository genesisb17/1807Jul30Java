...
public class UserDaoImpl implements UserDao {
	...
	public User getUser(String username) {
		int index = 0;
		try(Connection conn = ConnectionUtil.getConnection()){
			PreparedStatement ps = conn.prepareCall("select * from example_ysers where username = ?");
	...
	public static void main(String[] args) {
		System.out.println(UserDaoImpl.getInstance().getUserInformation("tiffannieM").toString());
	}
}
