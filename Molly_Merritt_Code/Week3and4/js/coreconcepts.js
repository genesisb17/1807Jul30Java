...
// these variables are global scope b/c they're outside any function
// you can access any global variable through the console
var a = 5;
var b = "hello"
var c = 'hello';
var d = 7/0
...
let globalLet = "this is still a global variable";
...
function scopes(cond) {
    ...
    if(cond) {  // here just to show that we are declaring vars in a block
        var blockV = "block var";   // var can only ever be function or global scope
        let blockL = "block let";   // let and const allow the use of block scope
    ...
    // we can re-declare variables and it doesn't matter
    var functionVar;
    console.log(functionVar);

    noDeclaration = "this variable was never declared but is used in a function... what scope is it?";
    // noDeclaration will be global and will be hoisted to the top of the page
    // before we call the function, this is the same as typing "noDeclaration;" at the top of the page
    // only after we call scope(cond) is noDeclaration initialized
    // technically noDeclaration doesn't exist in this script until scope(cond) is called
...
// ----------------------------------------OPERATORS--------------------------------------------------------
/* GUARD OPERATOR &&
    - if the first operand is truthy, return the second operand, otherwise it returns the first operand
