...
-- use this instead of storing passwords as plain text
...
BEGIN
  insert_user('williamG', 'password', 'William', 'Gentry', 'william@test.com');
END;
/

select * from example_users u
inner join example_user_information i
on u.username = i.username;


select GET_USER_HASH('williamG', 'password') from dual;

SELECT * FROM example_users WHERE username = 'williamG';